...
function test_login {
  ...
  rm -f dumped-headers.txt
  curl --dump-header dumped-headers.txt --fail -o/dev/null http://localhost:8080/camunda/app/${1}/default/
  # dumped-headers.txt uses windows line endings, drop them
  curl --cookie dumped-headers.txt -H "$(cat dumped-headers.txt | grep X-XSRF-TOKEN | tr -d '\r\n')" -H "Accept: application/json" --fail --data 'username=demo&password=demo' -o/dev/null http://localhost:8080/camunda/api/admin/auth/user/default/login/${1}
}
